...
program define adtest, rclass
	...
	// run AD test
	mata: ad_by_test2("`depvar'", "`by'", "`touse'", `reps', `power')
    
    // display results
	di _n "Two-sample Anderson-Darling test for equality of distribution functions"
	di "{hline 71}"
	di "Outcome:  `depvar'"
...
void ad_by_test2(depvar, byvar, touse, reps, power)
{
	...
	for (r = 1; r <= reps; r++) {
		...
		stat = ad_compute(shuf[1::n1], shuf[(n1+1)::N], power)
		if (stat >= obs) count++
	}
...
real scalar ad_compute(a, b, pwr)
{
